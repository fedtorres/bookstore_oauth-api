...
import (
	"encoding/json"
	"fmt"
	"github.com/fedtorres/bookstore_oauth-api/src/domain/users"
...
func (r *usersRepository) LoginUser(email string, password string) (*users.User, rest_errors.RestErr) {
	...
	if err := json.Unmarshal(response.Bytes(), &user); err != nil {
		return nil, rest_errors.NewInternalServerError("error when trying to unmarshal users login response", errors.New("restclient error"))
	}
	return &user, nil
}
